...
class SecurityExchangeUser(HttpUser):
    ...
    def orderCancelScenario(self):
        ...
        side = random.choice(["buy", "sell"])
        order_response = self.post_authenticated("/api/v1/order", {"stock_id": STOCK_ID, "side": side, "price": price, "quantity": quantity})
        if order_response.status_code == 200:
            try:
                order_data = order_response.json()
                order_id = order_data.get('order_id')
                if order_id:
                    self.delete_authenticated(f"/api/v1/order/{order_id}")
            except Exception as e:
